...
//
//    @Query(nativeQuery = true, value = "select * from pm_ver where del_flg = 0")
//    List<PMVer> findByNotDeleted();
//
...
//
//    @Query(nativeQuery = true, value = "select t.* from pm_ver t where DATE(on_air_date) = ?1 and t.del_flg = 0 and FIND_IN_SET(?2, team_arr)")
